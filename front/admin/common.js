...
var view = {
    ...
    setButtonCollapse: function (switcher) {
        if (switcher) {
            let content = $('#pagination');
            let contentHTML = $('#patternCollapse').html().replace("{collapse}", "Свернуть");

            content.html(contentHTML);
        } else {
            let contentHTML = $('#pagination');
            contentHTML.empty();
        }
    },
    ...
    addBooksToTable: function (books) {
        var content = $('#table');
        var contentHTML = $('#table-head').html();
        for (let book of books) {
            contentHTML += view.addBookToTable(book);
        }
        content.html(contentHTML);
    },
    addBookToTable: function(book) {
        let deleteLink = $('#delete-pattern').html().replaceAll('{id}', book.id);
        deleteLink = book.action ? deleteLink.replaceAll('{bookStatus}', 'Восстановить') : 
            deleteLink.replaceAll('{bookStatus}', 'Удалить');
        return $('#pattern').html()
                    .replace('{title}', book.title)
                    .replace('{author}', book.author)
                    .replace('{visits}', book.visits)
                    .replace('{purchases}', book.purchases)
                    .replace('{action}', deleteLink);
    },
    setPagination: function(totalBooks, firstNum, lastNum) {
        var element = $('#pagination');
        var paginationPrevious = $('#pagination-previous');
        var paginationNext = $('#pagination-next');
        var contentHTML = firstNum > 1 ? paginationPrevious.html() : '';

        for(let i = firstNum; i <= lastNum; i++) {
            contentHTML += view.addPaginationNum(i);
        }
        
        contentHTML += totalBooks > lastNum * 4 ? paginationNext.html() : '';

        element.html(contentHTML);
    },
    addPaginationNum: function(pagNum) {
        return $('#pagination-pattern').html()
                    .replaceAll('{page}', pagNum);
    },
    changeTextDeleteLink: function(id, bookStatus) {
        let element = $(`#delete${id}`);
        if (bookStatus) {
            element.html('Восстановить');
        } else {
            element.html('Удалить');
        }
    }
};
/* ------------------------------- end view ----------------------------------*/
